...
output=$(python3 /ofbiz/metering.py 2>&1)

echo "Output from metering.py:"
echo "$output"

if [[ "$output" == *"Usage registered successfully!"* ]]; then
  set -x
  set -e
  ...
  _main "$@"
    ...
    echo "Error: Metering API failed. Application not started."
    exit 1
fi
